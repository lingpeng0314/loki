...
func NewScheduler(cfg Config, limits Limits, log log.Logger, registerer prometheus.Registerer) (*Scheduler, error) {
	...
	// If a scheduler ring is configured we will build a ring for schedulers
	if cfg.SchedulerRing != nil {
...
// ReadRing returns the scheduler ring as a ReadRing interface, it returns nil if not ring is defined.
